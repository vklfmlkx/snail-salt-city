...
“测试功能”中的自定义行动默认关闭。启用后，每次预览最多调用两次模型：一次写局部插曲，一次审查与原剧情的衔接；格式或审查失败不消耗游戏行动，也不自动重试，但已发生的 API 请求计入预算。`LLM_CUSTOM_TIMEOUT_MS` 默认45000毫秒，`LLM_CUSTOM_MAX_OUTPUT_TOKENS` 默认4096，审查共用叙述角色的超时配置。上述调用受个人／全局调用次数、并发与累计金额限制，与每日6次故事生成额度分别计量。Mock 只回放固定内容，不会理解玩家的新做法。
