...
export function mountain(seed: number): Platform[] {
  const r = { random: seed >>> 0 };
  const result: Platform[] = [{ x: 0, y: 215, w: 106 }];
  for (let i = 1; i < 9; i++) {
    const p = result[i - 1];
    // Every landing is wide enough to stop; gaps/heights are well inside a
    // normal jump's envelope. Air dash is optional, never a required exploit.
    result.push({
      x: p.x + p.w + 30 + random(r, 21),
      y: Math.max(130, Math.min(232, p.y + random(r, 49) - 24)),
      w: 80 + random(r, 27),
    });
  }
  return result;
}
export function skyway(seed: number): Gate[] {
  const r = { random: seed >>> 0 };
  let y = 145;
  return Array.from({ length: 10 }, (_, i) => {
    y = Math.max(100, Math.min(194, y + random(r, 41) - 20));
    return { x: 260 + i * 160, y, gap: 108 + random(r, 17) };
  });
}
...
export function initialPixel(game: PixelGame, seed: number): PixelState {
  const s: PixelState = {
    game,
    seed,
    random: seed >>> 0,
  ...
  if (game === "summit") s.platforms = mountain(seed);
  if (game === "flight") s.gates = skyway(seed);
...
export function opponentMove(s: PixelState) {
  ...
  shot(s, d.known[1] === 0 || (d.known[1] === null && live === 0));
}
export function decision(s: PixelState, move: number) {
  if (s.finished || !Number.isInteger(move) || move < 0 || move > 4) {
    s.valid = false;
    return;
  }
  s.tick++;
  if (s.game === "roulette") {
    ...
    else if (move === 4 || !useItem(s, move - 2)) {
      s.valid = false;
      return;
    }
    let steps = 0;
    ...
    if (s.stunned > 0) s.stunned--;
      ...
      s.guards = s.guards.map((g, i) => {
        const directions = i % 2 ? [16, 1, -16, -1] : [1, 16, -1, -16];
        const start = (s.tick / 2 + i + s.seed) % 4;
        for (let n = 0; n < 4; n++) {
          const next = g + directions[(start + n) % 4];
          if (!s.walls.includes(next) && next > 16 && next < 143) return next;
        }
        return g;
      });
...
export function playPixel(game: PixelGame, seed: number, moves: number[]) {
  const s = initialPixel(game, seed);
  if (moves.length > 512) {
    s.valid = false;
    return s;
  }
