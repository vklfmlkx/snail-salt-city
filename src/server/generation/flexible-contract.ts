...
export function flexibleWriterPrompt(style: string, sourceText: string) {
  return [
    ...
    "在适合喘息、整理、观察或协作的位置加入activity，每幕至多一个，不在紧迫追逐中强插。activity={attribute,game,title,afterLine,intro,success,failure}。game仅body:summit、agility:flight、mind:roulette、presence:rally，按情境选择。在opening中自然描写剧中人物即将进行的具体小动作，不能提游戏名称、跑团桌或猫咪城主，afterLine指向说明的最后一段，玩家下一次推进直接打开游戏，完成一局才能续读。intro保留这两至六段用于契约记录，不要在正文中重复播放；success/failure是2至6段结果，成功有限+1、失败无惩罚，无主线事实变化。不是考试题，不要求玩家答知识问答。",
    "输出字段仅title,description,graphFlags,initialFlags,flagLabels,roleNames,stages,endings。stages每项：title,location,roles,anchor,knownFacts,landing,sideLimit:0,opening,transition:[],choices,可选activity。choices每项：id,attribute,tier,label,conditions,risk,branches,routes。routes的success/partial/failure分别{stage或ending二选一,grants:[],bridge:[],可选requires和otherwise}。条件结局仅ending可用requires，必须给otherwise。stage编号从1起。endings每项{id,category:good|bad|true,title,dialogue}。其他字段格式遵守所给JSON契约。",
